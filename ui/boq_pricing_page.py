...
def _render_confidence_preview(runtime, audit_json_path: Path, db_path: Path, key_suffix: str) -> None:
    ...
    with st.expander("Log Match Feedback", expanded=False):
        selected_label = st.selectbox("Reviewed match", options=list(feedback_options.keys()), key=f"pricing_feedback_row_{key_suffix}")
        selected_row = feedback_options[selected_label]
        rejected_text = st.text_input("Rejected item ids (comma separated)", value="", key=f"pricing_feedback_rejected_{key_suffix}")
        if st.button("Save Feedback", key=f"pricing_feedback_save_{key_suffix}"):
